...
void Solver::acceleratedRSPlanner(const State &from, const State &to, double &d,
  ...
  float theta0g;
  if (offset) {
    theta0g = from.theta + pi / 2;
  } else {
    theta0g = from.theta;
  }
  const double xng = to.x;
  const double yng = to.y;
  const float thetafg = to.theta;
...
void Solver::getPath(const int condition, char motionTypes[5],
  ...
  //     {static_cast<int>(x), static_cast<int>(y), wrapToPi(t)});
  for (int k = 0; k < 5; ++k) {
    ...
    primitives_.motionTypes.push_back(motionType);
    primitives_.motionLengths.push_back(motionLength * r);
    primitives_.motionDirections.push_back(motionDirection);
    primitives_.waypoints.push_back(
        {static_cast<int>(x), static_cast<int>(y), wrapToPi(t)});
  }
}
